...
(function(window)
{
	...
	CRunParagraphMark.prototype.Measure = function(oMeasurer, oTextPr, isEndCell)
	{
		...
		//	let nUnicode  = isEndCell ? 0x00A4 : 0x21B5; // 00B6  
		//  不论段落换行还是表格换行一律用回车符
		let nUnicode  = 0x21B5; 
		...
		//原本onlyoffice的换行符是根据Unicode码在当前字体（oFontInfo）库中进行字符搜索
  		//0x21B5不是所有字体都含有，虽然onlyoffice有字体回退机制，但大部分字体仍会显示空白框
   		//因此这里强制使用'DejaVu Math TeX Gyre'的↵作为换行符//因此这里强制使用'DejaVu Math TeX Gyre'的↵作为换行符
		this.Grapheme = oMeasurer.GetGraphemeByUnicode(nUnicode,'DejaVu Math TeX Gyre', oFontInfo.Style);
		//this.Grapheme = oMeasurer.GetGraphemeByUnicode(nUnicode, oFontInfo.Name, oFontInfo.Style);
		
		let nFontSize = oFontInfo.Size*0.7;
